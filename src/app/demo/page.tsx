...
export default function DemoPage() {
  return (
    ...
    >
      {/* ═══ Navigation ═══ */}
      <nav
        className="sticky top-0 z-50 h-[60px] bg-white/95 border-b border-cy-line flex items-center px-6 md:px-8 gap-4"
      ...
      {/* ═══ Hero ═══ */}
      <section
        className="relative overflow-hidden pt-16 pb-14 md:pt-24 md:pb-20 border-b border-cy-line"
        style={{
          background: `
      ...
      >
        <div className="relative z-10 max-w-[1280px] mx-auto px-6 text-center">
          <Reveal delay={0.05}>
            <span className="inline-flex items-center gap-2 px-3 py-1.5 bg-cy-navy-pale text-cy-navy rounded-full text-[12px] font-semibold mb-6">
          ...
          <Reveal delay={0.1}>
            ...
            >
              <span className="block text-cy-ink">6개 역할을</span>
              <span className="block text-cy-navy">직접 조작해 본다</span>
            </h1>
          </Reveal>

          <Reveal delay={0.25}>
            <p
              className="text-[16px] md:text-[18px] font-semibold text-cy-ink-2 mb-3"
              style={{ letterSpacing: "-0.02em" }}
            >
              전용콜 → 콜센터 → 공유콜. 3단계 폴백 자동화.
            </p>
            <p className="text-[14px] text-cy-ink-3 max-w-2xl mx-auto mb-10">
      ...
      {/* ═══ Equipment 8종 ═══ */}
      <section className="py-12 border-b border-cy-line bg-white">
        <div className="max-w-[1280px] mx-auto px-6">
          <Reveal>
            <p
              className="text-center text-[11px] text-cy-ink-3 font-medium mb-5 tracking-[0.2em] uppercase"
              style={{ fontFamily: "var(--font-roboto-mono), monospace" }}
            >
          ...
          <StaggerContainer className="flex flex-wrap justify-center gap-5 md:gap-8">
            {EQUIPMENT.map(({ name, Icon }) => (
              <StaggerItem key={name} className="flex flex-col items-center gap-2 group">
                <div className="w-12 h-12 flex items-center justify-center rounded-xl bg-cy-navy-pale text-cy-navy group-hover:bg-cy-navy group-hover:text-white transition-colors">
                  <Icon size={26} />
                </div>
                <span className="text-[11px] text-cy-ink-3 group-hover:text-cy-navy transition-colors font-medium">
                  {name}
                </span>
              </StaggerItem>
            ))}
      ...
      {/* ═══ Role Selection (핵심) ═══ */}
      <section id="roles" className="py-16 md:py-20 bg-cy-bg">
        <div className="max-w-[1280px] mx-auto px-6">
          <Reveal>
            <p
            ...
            <h2 className="text-[26px] md:text-[36px] font-[800] text-cy-ink tracking-[-0.02em] mb-2">
              체험할 역할 선택
            </h2>
              ...
              <StaggerItem key={role.id}>
                <Link href={`/demo/${role.id}`} className="block group">
                  <MotionCard className="p-6 h-full relative overflow-hidden">
                    {role.navyAccent && (
                      <div
                        className="absolute top-0 right-0 w-32 h-32 bg-cy-navy opacity-[0.04] rounded-full -translate-y-1/2 translate-x-1/2 pointer-events-none"
                        aria-hidden="true"
                      />
                    )}
                    ...
                    <h3
                      className="text-[18px] font-[800] text-cy-ink mb-1"
                      style={{ letterSpacing: "-0.02em" }}
                    >
                      {role.label}
                    </h3>
                    <p className="text-[13px] text-cy-ink-3 mb-4">{role.desc}</p>
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-cy-line">
                      <span className="text-[13px] text-cy-navy font-bold flex items-center gap-1 group-hover:translate-x-0.5 transition-transform">
                        체험하기
                      ...
                      <span
                        className="text-[10px] text-cy-ink-4"
                        style={{ fontFamily: "var(--font-roboto-mono), monospace" }}
                      >
                        /{role.id}
                      </span>
            ...
            <StaggerItem>
              <Link href="/demo/field-manager" className="block group">
                <MotionCard className="p-6 relative overflow-hidden h-full">
                  <div
                    aria-hidden="true"
                    className="absolute top-0 right-0 w-40 h-40 bg-cy-navy opacity-[0.04] rounded-full -translate-y-1/2 translate-x-1/2 pointer-events-none"
                  />
                  <div className="flex items-center gap-3 mb-4 relative">
                    <div className="w-12 h-12 rounded-xl bg-cy-navy-pale text-cy-navy flex items-center justify-center">
                      <span
                        className="material-symbols-outlined"
                        style={{ fontSize: 24, fontVariationSettings: "'FILL' 1" }}
                      >
                    ...
                    <div>
                      <span
                        className="text-[10px] px-2 py-0.5 bg-cy-bg text-cy-ink-3 rounded-md inline-block mb-0.5"
                        style={{ fontFamily: "var(--font-roboto-mono), monospace" }}
                      >
                        Mobile 1 · Phone
                      </span>
                      <h3
                        className="text-[18px] font-[800] text-cy-ink"
                        style={{ letterSpacing: "-0.02em" }}
                      >
                        현장소장 배차 화면
                      </h3>
                  ...
                  <p className="text-[13px] text-cy-ink-2 leading-[1.7] mb-4">
            ...
            <StaggerItem>
              <Link href="/demo/executive" className="block group">
                <MotionCard className="p-6 relative overflow-hidden h-full">
                  <div
                    aria-hidden="true"
                    className="absolute top-0 right-0 w-40 h-40 bg-cy-cyan opacity-[0.05] rounded-full -translate-y-1/2 translate-x-1/2 pointer-events-none"
                  />
                  <div className="flex items-center gap-3 mb-4 relative">
                    <div className="w-12 h-12 rounded-xl bg-cy-navy-pale text-cy-navy flex items-center justify-center">
                      <span
      ...
      {/* ═══ 6-Step Flow ═══ */}
      <section className="py-16 md:py-20 bg-cy-bg border-y border-cy-line">
        <div className="max-w-[1280px] mx-auto px-6">
          <Reveal>
            <p
              className="text-[11px] text-cy-navy font-bold tracking-[0.2em] uppercase mb-2.5"
              ...
              <StaggerItem key={step.num}>
                <MotionCard className="p-6 h-full relative">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {step.highlight && (
                      <span
                        className="px-2 py-1 bg-cy-warning/15 border border-cy-warning/40 rounded-full text-[11px] font-bold text-cy-warning-deep animate-pulse"
                        style={{ fontFamily: "var(--font-roboto-mono), monospace" }}
                      >
                        ⏱ {step.highlight}
                      </span>
                  ...
                  <h3
                    className="text-[18px] font-bold text-cy-ink mb-2"
                    style={{ letterSpacing: "-0.02em" }}
                  >
                    {step.title}
                  </h3>
                  <p className="text-[13px] text-cy-ink-2 leading-[1.7] mb-4">{step.desc}</p>

                  <div className="pt-4 border-t border-cy-line">
                    <span
                      className="text-[10px] uppercase tracking-[0.15em] text-cy-ink-3 block mb-1"
                      style={{ fontFamily: "var(--font-roboto-mono), monospace" }}
                    >
                      Actor
                    </span>
                    <span className="text-[13px] font-bold text-cy-ink">{step.actor}</span>
                  </div>
                </MotionCard>
              </StaggerItem>
            ))}
      ...
      {/* ═══ Final CTA ═══ */}
      <section className="py-16 md:py-20 bg-cy-bg">
        <div className="max-w-[960px] mx-auto px-6 text-center">
          <Reveal>
            <h2 className="text-[26px] md:text-[32px] font-[800] text-cy-ink tracking-[-0.02em] mb-3">
              이제 직접 체험해 본다
            </h2>
            <p className="text-[14px] md:text-[15px] text-cy-ink-3 mb-8">
      ...
      <footer className="border-t border-cy-line py-8 bg-cy-bg-alt">
        <div className="max-w-[1280px] mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-3">
          <div className="flex items-baseline gap-2.5">
            ...
            <span className="text-[11px] text-cy-ink-3 ml-2">BYTEFORCE (바이트포스)</span>
          </div>
