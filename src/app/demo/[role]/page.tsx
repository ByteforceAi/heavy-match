...
function OwnerDemo() {
  ...
  const matchedCalls = DEMO_DISPATCHES.filter((d) => d.status === "matched");
  ...
  return (
    <div className="space-y-6">
      ...
      <section>
        ...
        {exclusiveCalls.map((c) => (
          <Card key={c.id} className="mb-3">
            {c.exclusive_call_at && (
              <div className="mb-3"><CountdownTimer startedAt={c.exclusive_call_at} /></div>
            )}
            <div className="flex items-center justify-between mb-2">
              <span className="font-bold text-lg">{c.equipment_types.name} {c.equipment_specs.spec_name}</span>
              <StatusBadge status={c.status} />
            </div>
            <p className="text-sm text-text-muted">{c.site_address}</p>
            <p className="text-sm text-text-muted">{c.company_name}</p>
            <div className="flex items-center justify-between mt-3">
              <span className="text-xl font-bold tabular-nums text-primary">{formatPrice(c.price)}원</span>
              <span className="text-sm text-text-muted">{c.time_units.name}</span>
            </div>
            <div className="flex gap-2 mt-3">
              <button className="flex-1 py-3 bg-success text-white font-bold rounded-xl text-lg">수락</button>
              <button className="flex-1 py-3 bg-danger text-white font-bold rounded-xl text-lg">거절</button>
            </div>
          </Card>
        ))}
      </section>
      ...
      <section>
        ...
        {sharedCalls.map((c) => (
          <Card key={c.id} className="mb-3">
            ...
            <button className="w-full mt-3 py-3 bg-success text-white font-bold rounded-xl text-lg">수락</button>
          </Card>
        ))}
      </section>
      ...
      <section>
        <h3 className="text-lg font-semibold mb-3 flex items-center gap-2">
          <span className="w-3 h-3 rounded-full bg-success" /> 기사 배정 대기 ({matchedCalls.length})
        </h3>
        {matchedCalls.map((c) => (
          <Card key={c.id} className="mb-3">
            <div className="flex items-center justify-between mb-2">
            ...
            <p className="text-sm text-text-muted">{c.site_address}</p>
            <span className="text-xl font-bold tabular-nums text-primary">{formatPrice(c.price)}원</span>
            <button className="w-full mt-3 py-3 bg-primary text-white font-bold rounded-xl text-lg">👷 기사 배정하기</button>
          </Card>
        ))}
      </section>
    </div>
