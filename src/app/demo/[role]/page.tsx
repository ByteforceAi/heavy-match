...
function Md3Card({ children, className = "", glow = false, onClick }: { children: React.ReactNode; className?: string; glow?: boolean; onClick?: () => void }) {
  return (
    <div onClick={onClick} className={`bg-white rounded-2xl border p-5 transition-all duration-200 ${glow ? "border-[#002C5F]/30 ring-2 ring-[#002C5F]/10 shadow-lg" : "border-[#E3E8EF]/30 shadow-[0_2px_12px_rgba(17,28,41,0.04)]"} ${onClick ? "cursor-pointer active:scale-[0.98]" : ""} ${className}`}>
      {children}
    </div>
  );
}
...
function Md3Toast({ message }: { message: string }) {
  return (
    <div className="fixed top-20 left-1/2 -translate-x-1/2 z-[200] px-6 py-3 rounded-2xl shadow-2xl text-sm font-bold bg-[#0A1628] text-white animate-fade-in">
      ✅ {message}
    </div>
  );
}
...
function RequesterDemo() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-[#002C5F] to-[#0046A4] rounded-3xl p-6 text-white">
        <div className="absolute -top-10 -right-10 w-40 h-40 bg-white/5 rounded-full" />
        <h2 className="text-2xl font-[800] mb-1">장비 요청 시작</h2>
        <p className="text-white/70 text-sm mb-5">8종 장비를 60초 안에 배차한다</p>
        <a href="/demo/requester/request" className="inline-flex py-3 px-6 bg-white text-[#002C5F] font-bold rounded-xl shadow-lg active:scale-95 transition-all items-center gap-2">
          <span className="material-symbols-outlined text-xl">edit_note</span>장비 요청하기
        </a>
      </div>

      <section>
        <h3 className="text-lg font-bold text-[#0A1628] mb-3 flex items-center gap-2">
          <span className="material-symbols-outlined text-[#002C5F]" style={{ fontVariationSettings: "'FILL' 1" }}>pending</span>
          진행중 ({active.length})
        </h3>
        <div className="space-y-3">
          {active.slice(0, 3).map(d => (
            <Md3Card key={d.id} glow={d.status === "exclusive_call"}>
              ...
              <p className="text-xl font-black tabular-nums text-[#002C5F] mt-2">{formatPrice(d.price)}원</p>
            </Md3Card>
          ))}
        </div>
      </section>
      ...
      <section>
        <h3 className="text-lg font-bold text-[#0A1628] mb-3 flex items-center gap-2">
          <span className="material-symbols-outlined text-[#002C5F]">bolt</span>빠른 재주문
        </h3>
        <div className="space-y-2">
          {DEMO_CALL_HISTORY.map(h => (
            <button key={h.id} onClick={() => show(`${h.equipment_types.name} ${h.equipment_specs.spec_name} 재주문`)}
              className="w-full text-left bg-white rounded-2xl p-4 border border-[#E3E8EF]/30 shadow-sm hover:border-[#002C5F]/30 transition-all active:scale-[0.98] group">
              <div className="flex items-center justify-between">
                <div>
                  <span className="font-bold text-[#0A1628]">{h.equipment_types.name} {h.equipment_specs.spec_name}</span>
                  <p className="text-sm text-[#3A4A5F] mt-0.5">{h.site_address}</p>
                </div>
                <span className="material-symbols-outlined text-[#002C5F] group-hover:translate-x-1 transition-transform">arrow_forward</span>
              </div>
            </button>
          ))}
        </div>
...
function OwnerDemo() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-[800] text-[#0A1628]">콜 수신 현황</h2>
        <div className="flex gap-1.5 text-xs">
          <span className="px-2 py-1 bg-[#E8F1FB] text-[#002C5F] rounded-lg font-bold">전용 {exclusive.length}</span>
      ...
      {exclusive.length > 0 && (
        <section>
          <h3 className="text-base font-bold text-[#0A1628] mb-3 flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-[#002C5F] animate-pulse" />전용콜
          </h3>
          {exclusive.map(c => (
            <Md3Card key={c.id} glow className="mb-3">
              ...
              <div className="flex items-center justify-between mb-2">
                <span className="font-bold text-lg text-[#0A1628]">{EQ_ICONS[c.equipment_types.name]} {c.equipment_types.name} {c.equipment_specs.spec_name}</span>
                <Md3Badge label="전용콜" color="bg-[#E8F1FB] text-[#002C5F]" />
              </div>
              <p className="text-sm text-[#3A4A5F]">{c.site_address}</p>
              <p className="text-sm text-[#6B7B8F]">{c.company_name}</p>
              <div className="flex items-center justify-between mt-3">
                <span className="text-xl font-black tabular-nums text-[#002C5F]">{formatPrice(c.price)}원</span>
                <span className="text-sm text-[#6B7B8F]">{c.time_units.name}</span>
              </div>
              <div className="flex gap-2 mt-4">
                <button
                  onClick={() => handleAccept(c.id, c.equipment_types.name)}
                  disabled={processing !== null}
                  className="flex-1 py-3.5 bg-[#002C5F] text-white font-bold rounded-xl text-base active:scale-95 transition-all flex items-center justify-center gap-1 disabled:bg-[#9AA8B8] disabled:cursor-wait relative overflow-hidden"
                >
          ...
          {shared.map(c => (
            <Md3Card key={c.id} className="mb-3">
              ...
              <button onClick={() => { setAccepted(p => new Set(p).add(c.id)); show("수락 완료"); }}
                className="w-full mt-3 py-3.5 bg-[#002C5F] text-white font-bold rounded-xl text-base active:scale-95 transition-all flex items-center justify-center gap-1">
                <span className="material-symbols-outlined text-lg">check_circle</span>수락
              </button>
            </Md3Card>
          ))}
          ...
          {matched.map(c => (
            <Md3Card key={c.id} className="mb-3">
              <div className="flex items-center gap-2 mb-2">
                <span className="font-bold text-[#0A1628]">{EQ_ICONS[c.equipment_types.name]} {c.equipment_types.name} {c.equipment_specs.spec_name}</span>
                <Md3Badge label="매칭완료" color="bg-emerald-100 text-emerald-700" />
              </div>
              <p className="text-sm text-[#3A4A5F]">{c.site_address}</p>
              <p className="text-xl font-black tabular-nums text-[#002C5F] mt-2">{formatPrice(c.price)}원</p>
              <button onClick={() => setAssignModal(c.id)}
                className="w-full mt-3 py-3.5 bg-[#0A1628] text-white font-bold rounded-xl text-base active:scale-95 transition-all flex items-center justify-center gap-2">
      ...
      {exclusive.length === 0 && shared.length === 0 && matched.length === 0 && (
        <Md3Card className="text-center py-12">
          <span className="material-symbols-outlined text-5xl text-[#002C5F] block mb-3" style={{ fontVariationSettings: "'FILL' 1" }}>celebration</span>
          <h3 className="text-xl font-[800] text-[#0A1628]">모든 콜을 처리했습니다!</h3>
          <p className="text-[#3A4A5F] mt-1">새 콜이 오면 알림을 수신한다</p>
        </Md3Card>
      )}
      ...
      {assignModal && (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center">
          ...
          <div className="relative z-10 bg-white rounded-t-3xl md:rounded-3xl w-full max-w-md p-6 shadow-2xl safe-bottom animate-fade-in">
            <div className="w-10 h-1 bg-[#E3E8EF] rounded-full mx-auto mb-4 md:hidden" />
            <h3 className="text-xl font-[800] text-[#0A1628] mb-4">기사 선택</h3>
            <div className="space-y-2">
              {DEMO_OPERATORS.map(op => (
                <button key={op.id} onClick={() => { setAssigned(p => new Set(p).add(assignModal)); setAssignModal(null); show(`${op.name} 기사 배정 완료`); }}
                  className="w-full flex items-center justify-between p-4 bg-[#F4F6FA] rounded-2xl hover:bg-[#E8F1FB] transition-all active:scale-[0.98]">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      <p className="font-bold text-[#0A1628]">{op.name}</p>
                      <p className="text-sm text-[#6B7B8F]">{op.phone}</p>
                    </div>
                  </div>
                  <span className="px-3 py-1.5 bg-[#002C5F] text-white rounded-xl text-sm font-bold">배정</span>
                </button>
              ))}
            </div>
            <button onClick={() => setAssignModal(null)} className="w-full mt-4 py-3 text-[#6B7B8F] font-medium">닫기</button>
          </div>
        </div>
      )}
    </div>
...
function OperatorDemo() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      {toast && <Md3Toast message={toast} />}
      <h2 className="text-2xl font-[800] text-[#0A1628]">현재 배차</h2>
      {dispatches.length > 0 ? dispatches.map(d => {
        const isStarted = started.has(d.id) || d.status === "in_progress";
        return (
          <Md3Card key={d.id} glow={isStarted}>
            <div className="flex items-center justify-between mb-2">
              <span className="font-bold text-lg text-[#0A1628]">{EQ_ICONS[d.equipment_types.name]} {d.equipment_types.name} {d.equipment_specs.spec_name}</span>
              <Md3Badge label={isStarted ? "작업중" : "기사배정"} color={isStarted ? "bg-amber-100 text-amber-700" : "bg-[#E8F1FB] text-[#002C5F]"} />
            </div>
            <p className="text-sm text-[#3A4A5F]">{d.site_address}</p>
            <p className="text-xl font-black tabular-nums text-[#002C5F] mt-2">{formatPrice(d.price)}원</p>
            {d.site_manager_phone && (
              <div className="mt-2 px-3 py-2 bg-[#F4F6FA] rounded-xl inline-flex items-center gap-1 text-sm text-[#002C5F] font-medium">
            ...
            {!isStarted ? (
              <button
                ...
                disabled={processing !== null}
                className="w-full mt-3 py-3.5 bg-amber-500 hover:bg-amber-600 text-white font-bold rounded-xl text-base active:scale-95 flex items-center justify-center gap-2 disabled:bg-[#9AA8B8] disabled:cursor-wait transition-colors"
              >
              ...
              <div className="mt-3 space-y-3 animate-fade-in">
                <p className="text-sm font-bold text-[#0A1628]">기사 전자서명</p>
        ...
        <Md3Card className="text-center py-12">
          ...
          <p className="text-sm text-[#6B7B8F] mt-1">오늘 배정된 작업을 모두 마쳤다</p>
        </Md3Card>
      )}
    ...
    <div className="space-y-5">
      ...
      <div className="lg:grid lg:grid-cols-[280px_1fr] lg:gap-6">
        ...
        <div className="lg:sticky lg:top-4 lg:self-start space-y-4 mb-5 lg:mb-0">
          <div>
            <h2 className="text-2xl font-[800] text-[#0A1628]">콜 관리</h2>
            <p className="text-sm text-[#3A4A5F]">미수락 콜 관리 · 사장 배정</p>
          </div>

          {/* 통계 필터 (세로 스택 — 데스크탑) */}
          <div className="grid grid-cols-4 lg:grid-cols-1 gap-2">
            {[{k:"all",l:"전체",g:"from-[#002C5F] to-[#0046A4]"},{k:"callcenter_call",l:"대기",g:"from-amber-500 to-orange-600"},{k:"shared_call",l:"공유콜",g:"from-violet-500 to-purple-600"},{k:"matched",l:"매칭완료",g:"from-emerald-500 to-green-600"}].map(f => (
              <button key={f.k} onClick={() => setFilter(f.k)}
                className={`${filter===f.k ? `bg-gradient-to-br ${f.g} text-white` : "bg-white border border-[#E3E8EF]/30 text-[#0A1628]"} rounded-xl p-3 text-center transition-all active:scale-95`}>
        ...
        <div className="space-y-3">
          <div className="hidden lg:flex items-center justify-between mb-2">
            <h3 className="text-lg font-bold text-[#0A1628]">콜 목록 ({filtered.length}건)</h3>
          </div>
          {filtered.map(d => (
            <Md3Card key={d.id} glow={d.status==="callcenter_call"}>
              ...
              <p className="text-lg font-black tabular-nums text-[#002C5F] mt-1">{formatPrice(d.price)}원</p>
              {d.status === "callcenter_call" && (
                <div className="flex gap-2 mt-3">
                  ...
                  <button onClick={() => { setCalls(prev => prev.map(c => c.id===d.id ? {...c,status:"shared_call"} : c)); show("공유콜로 전환됨"); }}
                    className="flex-1 py-3 bg-[#002C5F] text-white font-bold rounded-xl active:scale-95 transition-all flex items-center justify-center gap-1">
...
function SalespersonDemo() {
  ...
  return (
    <div className="space-y-5 max-w-2xl">
      {toast && <Md3Toast message={toast} />}
      <div>
        <h2 className="text-2xl font-[800] text-[#0A1628]">분양 현황</h2>
        <p className="text-sm text-[#3A4A5F]">나의 수수료 및 실적 현황</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Md3Stat icon="receipt_long" value={DEMO_COMMISSIONS.length} label="완료 건수" gradient="bg-gradient-to-br from-[#002C5F] to-[#0046A4]" />
      </div>
      ...
      <div className="flex gap-2">
        {([["all","전체"],["month","이번 달"],["week","이번 주"]] as const).map(([k,l]) => (
          <button key={k} onClick={() => setPeriod(k)}
            className={`px-4 py-2 rounded-xl text-sm font-bold transition-all active:scale-95 ${period===k ? "bg-[#002C5F] text-white" : "bg-white border border-[#E3E8EF]/30 text-[#3A4A5F]"}`}>
            {l}
          </button>
        ))}
      </div>
      ...
      <Md3Card className="text-center">
        <p className="text-sm text-[#6B7B8F]">{period==="all"?"전체":"필터된"} 수수료</p>
        <p className="text-3xl font-black tabular-nums text-emerald-600">{formatPrice(filteredTotal)}원</p>
        <p className="text-sm text-[#6B7B8F]">{filtered.length}건</p>
      </Md3Card>
      ...
      <div className="space-y-2">
        {filtered.map(c => (
          <Md3Card key={c.id} className="flex items-center justify-between" onClick={() => show(`거래 상세: ${formatPrice(c.total_price)}원`)}>
            <div>
              <p className="font-semibold text-[#0A1628]">{formatPrice(c.total_price)}원 건</p>
...
function AdminDemo() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-[800] text-[#0A1628]">관리자 대시보드</h2>
        <p className="text-sm text-[#3A4A5F]">철연 CHEOLYEON 플랫폼 현황 · 최근 30일</p>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Md3Stat icon="group" value={DEMO_ALL_USERS.length} label="총 사용자" gradient="bg-gradient-to-br from-[#002C5F] to-[#0046A4]" />
      ...
      <div className="grid grid-cols-3 gap-3">
        <Md3Card className="text-center !p-3">
          <p className="text-2xl font-black tabular-nums text-[#0A1628]">{DEMO_DISPATCHES.length}</p>
          <p className="text-[10px] text-[#6B7B8F] font-semibold">총 배차</p>
        </Md3Card>
        <Md3Card className="text-center !p-3">
          <p className="text-2xl font-black tabular-nums text-[#0A1628]">{formatPrice(totalVolume)}원</p>
          <p className="text-[10px] text-[#6B7B8F] font-semibold">총 거래액</p>
        </Md3Card>
        <Md3Card className="text-center !p-3">
          <p className="text-2xl font-black tabular-nums text-[#ba1a1a]">{cancelledCount}</p>
          <p className="text-[10px] text-[#6B7B8F] font-semibold">취소</p>
        </Md3Card>
      </div>
      {/* 차트 — 실제 데이터 기반 */}
      ...
      <section>
        <h3 className="text-lg font-bold text-[#0A1628] mb-3">최근 배차 (상위 10건)</h3>
        <div className="space-y-2">
          {DEMO_DISPATCHES.slice(0, 10).map(d => (
            <Md3Card key={d.id} className="flex items-center justify-between">
              <div>
                <p className="font-bold text-[#0A1628]">{EQ_ICONS[d.equipment_types.name]} {d.equipment_types.name} {d.equipment_specs.spec_name}</p>
                <p className="text-xs text-[#6B7B8F]">{d.company_name}</p>
              </div>
              <div className="text-right">
                <Md3Badge label={STATUS_BADGES[d.status]?.label || d.status} color={STATUS_BADGES[d.status]?.color} />
                <p className="text-sm text-[#3A4A5F] tabular-nums mt-1">{formatPrice(d.price)}원</p>
              </div>
            </Md3Card>
          ))}
        </div>
