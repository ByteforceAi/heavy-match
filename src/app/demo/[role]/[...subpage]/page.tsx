...
function Md3Card({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return <div className={`bg-white rounded-2xl border border-cy-line/30 p-5 shadow-[0_2px_12px_rgba(17,28,41,0.04)] ${className}`}>{children}</div>;
}
function Md3Badge({ label, color = "bg-cy-navy-pale text-cy-navy" }: { label: string; color?: string }) {
...
function Md3Toast({ message }: { message: string }) {
  return <div className="fixed top-20 left-1/2 -translate-x-1/2 z-[200] px-6 py-3 rounded-2xl shadow-2xl text-sm font-bold bg-cy-ink text-white animate-fade-in">✅ {message}</div>;
}
...
function PageTitle({ title, desc, action }: { title: string; desc?: string; action?: React.ReactNode }) {
  return (
    <div className="flex items-start justify-between mb-5">
      <div>
        <h2 className="text-2xl font-[800] text-cy-ink">{title}</h2>
        {desc && <p className="text-sm text-cy-ink-2 mt-0.5">{desc}</p>}
      </div>
      {action}
    </div>
  );
...
const STATUS_MAP: Record<string, { label: string; color: string }> = {
  exclusive_call: { label: "전용콜", color: "bg-cy-navy-pale text-cy-navy" },
  shared_call: { label: "공유콜", color: "bg-cy-line-soft text-cy-ink-3" },
  matched: { label: "매칭완료", color: "bg-emerald-100 text-emerald-700" },
  operator_assigned: { label: "기사배정", color: "bg-cy-navy-pale text-cy-navy" },
...
export default function DemoSubPage() {
  ...
  switch (key) {
    ...
    default: return <div className="text-center py-12 text-cy-ink-3"><span className="material-symbols-outlined text-5xl block mb-2">construction</span>{key}</div>;
  }
}
...
function DemoRequest() {
  ...
  return (
    <div className="max-w-md mx-auto space-y-5">
      ...
      <div className="flex gap-1">{[1,2,3,4,5,6].map(s => <div key={s} className={`flex-1 h-2 rounded-full transition-all ${s <= step ? "bg-cy-navy" : "bg-cy-line"}`} />)}</div>

      {step === 1 && (<>
        <h3 className="text-xl font-[800] text-cy-ink">장비 선택</h3>
        <div className="grid grid-cols-2 gap-3">
          {EQ.map(e => (
            <button key={e.id} onClick={() => { setEq(e); setStep(2); }}
              className="bg-white rounded-2xl p-4 text-center border border-cy-line/30 hover:border-cy-navy hover:shadow-md transition-all active:scale-95">
              <span className="text-3xl block mb-1">{e.icon}</span>
              <span className="text-base font-bold text-cy-ink">{e.name}</span>
            </button>
          ))}
        </div>
      </>)}
      ...
      {step === 2 && eq && (<>
        <h3 className="text-xl font-[800] text-cy-ink">{eq.icon} {eq.name} 규격</h3>
        <div className="grid grid-cols-2 gap-3">
          {(ALL_SPECS[eq.id]||[]).map(s => (
            <button key={s.id} onClick={() => { setSpec(s.name); setStep(3); }}
              className="bg-white rounded-2xl p-4 text-center border border-cy-line/30 hover:border-cy-navy transition-all active:scale-95">
              <span className="text-xl font-black text-cy-ink">{s.name}</span>
            </button>
          ))}
        </div>
        <button onClick={() => setStep(1)} className="text-sm text-cy-ink-3 flex items-center gap-1"><span className="material-symbols-outlined text-base">arrow_back</span>장비 다시 선택</button>
      </>)}

      {step === 3 && (<>
        <h3 className="text-xl font-[800] text-cy-ink">시간 선택</h3>
        <div className="grid grid-cols-2 gap-3">
          {TIMES.map(t => (
            <button key={t.id} onClick={() => { setTime(t.name); setStep(4); }}
              className="bg-white rounded-2xl p-4 text-center border border-cy-line/30 hover:border-cy-navy transition-all active:scale-95">
              <span className="text-lg font-bold text-cy-ink">{t.name}</span>
            </button>
          ))}
        </div>
        <button onClick={() => setStep(2)} className="text-sm text-cy-ink-3 flex items-center gap-1"><span className="material-symbols-outlined text-base">arrow_back</span>규격 다시 선택</button>
      </>)}

      {step === 4 && (<>
        <h3 className="text-xl font-[800] text-cy-ink">단가 확인</h3>
        <div className="bg-cy-bg rounded-2xl p-5">
          <p className="text-sm text-cy-ink-2">{eq?.name} {spec} / {time}</p>
          <input type="text" inputMode="numeric" value={price} onChange={e => setPrice(e.target.value.replace(/\D/g,""))}
            className="w-full mt-2 px-4 py-3 text-2xl font-black text-right border border-cy-line rounded-xl tabular-nums focus:ring-2 focus:ring-cy-navy/30 focus:border-cy-navy" />
          <p className="text-xs text-cy-ink-3 text-right mt-1">원 (₩)</p>
        </div>
        <button onClick={() => setStep(5)} className="w-full py-4 bg-cy-navy text-white font-bold rounded-xl active:scale-95 transition-all flex items-center justify-center gap-2">
          <span className="material-symbols-outlined">arrow_forward</span>다음
        </button>
        <button onClick={() => setStep(3)} className="text-sm text-cy-ink-3 flex items-center gap-1"><span className="material-symbols-outlined text-base">arrow_back</span>이전</button>
      </>)}

      {step === 5 && (<>
        <h3 className="text-xl font-[800] text-cy-ink">현장 정보</h3>
        <input type="text" defaultValue="한양건설(주)" placeholder="건설사명 *" className="w-full px-4 py-3 border border-cy-line rounded-xl bg-white focus:ring-2 focus:ring-cy-navy/30" />
        <input type="text" defaultValue="서울시 강남구 삼성동 코엑스 신축현장" placeholder="현장주소 *" className="w-full px-4 py-3 border border-cy-line rounded-xl bg-white focus:ring-2 focus:ring-cy-navy/30" />
        <input type="text" defaultValue="김건설" placeholder="요청자명" className="w-full px-4 py-3 border border-cy-line rounded-xl bg-white focus:ring-2 focus:ring-cy-navy/30" />
        <input type="tel" defaultValue="010-1234-5678" placeholder="연락처" className="w-full px-4 py-3 border border-cy-line rounded-xl bg-white focus:ring-2 focus:ring-cy-navy/30" />
        <button onClick={() => setStep(6)} className="w-full py-4 bg-cy-navy text-white font-bold rounded-xl active:scale-95 transition-all flex items-center justify-center gap-2">
          <span className="material-symbols-outlined">draw</span>다음 (전자서명)
        </button>
        <button onClick={() => setStep(4)} className="text-sm text-cy-ink-3 flex items-center gap-1"><span className="material-symbols-outlined text-base">arrow_back</span>이전</button>
      </>)}

      {step === 6 && (<>
        <h3 className="text-xl font-[800] text-cy-ink">전자서명</h3>
        <div className="bg-cy-bg rounded-xl p-3 text-sm">
          <p><b>{eq?.name}</b> {spec} / {time}</p>
          <p className="text-cy-navy font-black tabular-nums text-lg">{formatPrice(parseInt(price)||0)}원</p>
        </div>
        ...
        {submitting !== "idle" && submitting !== "done" && (
          <div className="space-y-1.5 bg-cy-bg border border-cy-line/30 rounded-xl p-3">
            {[
              ...
              return (
                <div key={s.key} className="flex items-center gap-2 text-xs">
                  <span
                    ...
                    style={{
                      color: isPast ? "#00A86B" : isActive ? "#002C5F" : "#E3E8EF",
                  ...
                  <span className={isPast ? "text-emerald-600 line-through" : isActive ? "text-cy-navy font-bold" : "text-cy-line"}>
                    {s.label}
                  </span>
                </div>
              );
        ...
        <button onClick={() => setStep(5)} className="text-sm text-cy-ink-3 flex items-center gap-1"><span className="material-symbols-outlined text-base">arrow_back</span>이전</button>
      </>)}
    </div>
  );
}
        ...
        {dispatches.map(d => (
          <Md3Card key={d.id}>
            <div className="flex items-center justify-between mb-2">
              <span className="font-bold text-cy-ink">{EQ_ICONS[d.equipment_types.name]} {d.equipment_types.name} {d.equipment_specs.spec_name}</span>
              <Md3Badge label={STATUS_MAP[d.status]?.label||d.status} color={STATUS_MAP[d.status]?.color} />
            </div>
            <p className="text-sm text-cy-ink-2">{d.site_address}</p>
            <div className="flex items-center justify-between mt-2">
              <span className="font-black tabular-nums text-cy-navy">{formatPrice(d.price)}원</span>
              <span className="text-xs text-cy-ink-3">{new Date(d.created_at).toLocaleDateString("ko-KR")}</span>
            </div>
          </Md3Card>
        ))}
      </div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        ...
        <Md3Stat icon="receipt_long" value={DEMO_COMMISSIONS.length} label="적립 건수" gradient="bg-gradient-to-br from-cy-navy to-cy-navy-mid" />
      </div>
      <div className="space-y-2">
        {DEMO_COMMISSIONS.map(c => (
          <Md3Card key={c.id} className="flex items-center justify-between">
            <div>
              <p className="font-semibold text-cy-ink text-sm">{formatPrice(c.total_price)}원 건</p>
              <p className="text-xs text-cy-ink-3">{new Date(c.created_at).toLocaleDateString("ko-KR")}</p>
            </div>
    ...
    <div className="space-y-4">
      <PageTitle title="단가 설정" desc="장비×규격×시간 매트릭스" action={
        <button onClick={() => { setSaved(true); setTimeout(() => setSaved(false), 2000); }}
          className={`px-5 py-2.5 font-bold rounded-xl text-sm active:scale-95 transition-all ${saved ? "bg-emerald-600 text-white" : "bg-cy-navy text-white"}`}>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2">
        {TYPES.map(t => (
          <button key={t} onClick={() => setSel(t)}
            className={`px-4 py-2.5 rounded-xl whitespace-nowrap text-sm font-bold transition-all active:scale-95 ${sel === t ? "bg-cy-navy text-white shadow-md" : "bg-white border border-cy-line/50 text-cy-ink-2 hover:border-cy-navy"}`}>
            {t}
          </button>
        ))}
      </div>
      <div className="overflow-x-auto bg-white rounded-2xl border border-cy-line/30 shadow-sm">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-cy-line/30 bg-cy-bg">
            <th className="text-left p-3 font-bold text-cy-ink-2">규격</th>
            {TIMES.map(t => <th key={t} className="text-center p-3 font-bold text-cy-ink-2 whitespace-nowrap">{t}</th>)}
          </tr></thead>
          <tbody>{(ALL_SPECS[sel]||[]).map(spec => (
            <tr key={spec} className="border-b border-cy-line/10 hover:bg-cy-bg/50 transition-colors">
              <td className="p-3 font-black text-cy-ink">{spec}</td>
              {TIMES.map(t => { const k=`${sel}-${spec}-${t}`; return (
                <td key={t} className="p-1.5">
                  <input type="text" inputMode="numeric" value={prices[k]??""} onChange={e => setPrices(p => ({...p,[k]:e.target.value.replace(/\D/g,"")}))}
                    placeholder="0" className="w-full px-3 py-2.5 text-sm text-right border border-cy-line/50 rounded-xl tabular-nums focus:ring-2 focus:ring-cy-navy/30 focus:border-cy-navy transition-all" />
                </td>
              ); })}
            </tr>
          ))}</tbody>
  ...
  return (
    <div>
      ...
      <PageTitle title="소속 기사 관리" desc={`총 ${ops.length}명`} action={
        <button onClick={() => setShowAdd(!showAdd)} className="px-4 py-2.5 bg-cy-navy text-white font-bold rounded-xl text-sm active:scale-95 flex items-center gap-1">
      ...
      {showAdd && (
        <Md3Card className="mb-4 space-y-3">
          <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="기사 이름" className="w-full px-4 py-3 border border-cy-line rounded-xl focus:ring-2 focus:ring-cy-navy/30" />
          <input type="tel" value={phone} onChange={e => setPhone(e.target.value)} placeholder="010-0000-0000" className="w-full px-4 py-3 border border-cy-line rounded-xl focus:ring-2 focus:ring-cy-navy/30" />
          <button onClick={() => { if(!name) return; setOps(p => [...p,{id:`n${Date.now()}`,name,phone:phone||"010-0000-0000",created_at:new Date().toISOString()}]); setName(""); setPhone(""); setShowAdd(false); show(`${name} 등록 완료`); }}
            disabled={!name} className="w-full py-3 bg-cy-navy text-white font-bold rounded-xl disabled:opacity-40 active:scale-95">등록</button>
        </Md3Card>
      )}
      <div className="space-y-2">
        {ops.map(op => (
          <Md3Card key={op.id} className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-cy-navy-pale text-cy-navy rounded-full flex items-center justify-center font-bold">{op.name[0]}</div>
              <div><p className="font-bold text-cy-ink">{op.name}</p><p className="text-sm text-cy-ink-3">{op.phone}</p></div>
            </div>
...
function DemoInvite() {
  ...
  const url = "https://cheolyeon.com/register?ref=demo-own-1";
  ...
  return (
    <div>
      ...
      <Md3Card className="space-y-4">
        <div className="bg-cy-bg rounded-xl p-4">
          <p className="text-sm font-bold text-cy-navy mb-2 flex items-center gap-1"><span className="material-symbols-outlined text-base">link</span>초대 링크</p>
          <p className="text-xs text-cy-ink-3 break-all font-mono bg-white rounded-lg p-3 border border-cy-line/30 select-all">{url}</p>
        </div>
        <button onClick={copy} className={`w-full py-4 font-bold rounded-xl text-lg active:scale-95 transition-all flex items-center justify-center gap-2 ${copied ? "bg-emerald-600 text-white" : "bg-cy-navy text-white"}`}>
        ...
        {DEMO_OWNERS.map(o => (
          <Md3Card key={o.id} className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-bold text-cy-ink">{o.name}</p>
                <p className="text-sm text-cy-ink-3">{o.company_name}</p>
                <p className="text-xs text-cy-ink-3">{o.region_sido} {o.region_sigungu}</p>
              </div>
            </div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        ...
        <Md3Stat icon="receipt_long" value={DEMO_COMMISSIONS.length} label="건수" gradient="bg-gradient-to-br from-cy-navy to-cy-navy-mid" />
      </div>
      <div className="space-y-2">
        {DEMO_COMMISSIONS.map(c => (
          <Md3Card key={c.id} className="flex items-center justify-between">
            <div>
              <p className="font-semibold text-cy-ink text-sm">거래 {formatPrice(c.total_price)}원</p>
              <p className="text-xs text-cy-ink-3">{new Date(c.created_at).toLocaleDateString("ko-KR")}</p>
            </div>
      ...
      <div className="space-y-2">
        {DEMO_DISPATCHES.map(d => (
          <Md3Card key={d.id}>
            ...
            <p className="text-sm text-cy-ink-2">{d.company_name} · {d.site_address}</p>
            <div className="flex items-center justify-between mt-2">
              <span className="font-black tabular-nums text-cy-navy">{formatPrice(d.price)}원</span>
              <span className="text-xs text-cy-ink-3">{new Date(d.created_at).toLocaleString("ko-KR")}</span>
            </div>
          </Md3Card>
        ))}
      </div>
    ...
    <div>
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-2 mb-6">
        {Object.entries(labels).map(([role,label]) => (
          <div key={role} className="bg-white rounded-xl p-3 text-center border border-cy-line/30 shadow-sm">
            <p className="text-xl font-black tabular-nums text-cy-ink">{roleCounts[role]??0}</p>
            <p className="text-[10px] text-cy-ink-3 font-semibold">{label}</p>
          </div>
        ))}
      </div>
      <div className="space-y-2">
        {DEMO_ALL_USERS.map(u => (
          <Md3Card key={u.id} className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 bg-cy-bg rounded-full flex items-center justify-center text-sm font-bold text-cy-navy">{u.name[0]}</div>
              <div>
                <span className="font-bold text-cy-ink">{u.name}</span>
                {u.company_name && <span className="text-sm text-cy-ink-3 ml-2">{u.company_name}</span>}
              </div>
            </div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        ...
        <Md3Stat icon="redeem" value={`${formatPrice(t.rq)}원`} label="건설사 적립" gradient="bg-gradient-to-br from-cy-navy to-cy-navy-mid" />
      </div>
      <div className="space-y-2">
        {DEMO_COMMISSIONS.map(c => (
          <Md3Card key={c.id}>
            <div className="flex items-center justify-between mb-1">
              <span className="font-semibold text-sm text-cy-ink">거래 {formatPrice(c.total_price)}원</span>
              <span className="text-xs text-cy-ink-3">{new Date(c.created_at).toLocaleDateString("ko-KR")}</span>
            </div>
            <div className="grid grid-cols-4 gap-1 text-xs text-center">
              <div className="bg-emerald-50 rounded-lg p-1.5"><p className="text-cy-ink-3">본사</p><p className="font-black tabular-nums">{formatPrice(c.company_fee)}</p></div>
              <div className="bg-violet-50 rounded-lg p-1.5"><p className="text-cy-ink-3">콜센터</p><p className="font-black tabular-nums">{formatPrice(c.callcenter_fee)}</p></div>
              <div className="bg-pink-50 rounded-lg p-1.5"><p className="text-cy-ink-3">영업</p><p className="font-black tabular-nums">{formatPrice(c.salesperson_fee)}</p></div>
              <div className="bg-blue-50 rounded-lg p-1.5"><p className="text-cy-ink-3">적립</p><p className="font-black tabular-nums">{formatPrice(c.requester_reward)}</p></div>
            </div>
          </Md3Card>
        ))}
      </div>
  ...
  return (
    <div>
      ...
      <Md3Card className="mb-4">
        <h3 className="font-[800] text-lg text-cy-ink mb-3">장비 종류 (8종)</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          {TYPES.map(t => (
            <button key={t.name} onClick={() => setSel(sel === t.name ? null : t.name)}
              className={`p-3 rounded-xl border-2 text-center transition-all active:scale-95 ${sel === t.name ? "border-cy-navy bg-cy-bg" : "border-cy-line/30 hover:border-cy-navy"}`}>
              <span className="text-2xl block">{t.icon}</span>
              <span className="text-sm font-bold text-cy-ink">{t.name}</span>
            </button>
          ))}
        </div>
      </Md3Card>
      {sel && (
        <Md3Card className="mb-4 animate-fade-in">
          <h3 className="font-[800] text-lg text-cy-ink mb-3">{sel} 규격</h3>
          <div className="flex flex-wrap gap-2">
            {TYPES.find(t => t.name === sel)?.specs.map(s => (
              <span key={s} className="px-4 py-2 bg-cy-navy-pale text-cy-navy rounded-xl text-sm font-bold">{s}</span>
            ))}
          </div>
        </Md3Card>
      )}
      <Md3Card>
        <h3 className="font-[800] text-lg text-cy-ink mb-3">시간 단위</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          {TIMES.map(t => (
            <div key={t.name} className="p-3 bg-cy-bg rounded-xl text-center">
              <span className="font-bold text-cy-ink">{t.name}</span>
              <p className="text-xs text-cy-ink-3">{t.h}시간</p>
            </div>
          ))}
        </div>
      </Md3Card>
