  ...
  return (
    ...
    >
      ...
      <section className="max-w-[1400px] mx-auto px-6 md:px-12 pt-10 pb-6">
        ...
        <h1 className="text-[32px] md:text-[44px] font-[800] tracking-[-0.02em] text-cy-ink mb-2">
          Skeleton Showcase
        </h1>
        <p className="text-[14px] md:text-[15px] text-cy-ink-3 mb-6 max-w-[640px]">
          내부 개발용 — 로딩 상태 프리미티브 카탈로그다. 각 컴포넌트의 실제 렌더링과
          호출 방법을 한 화면에서 확인한다.
        </p>

        <label className="inline-flex items-center gap-2.5 px-4 py-2.5 bg-white border border-cy-line rounded-xl cursor-pointer select-none">
          <input
            type="checkbox"
            checked={reduced}
            onChange={(e) => setReduced(e.target.checked)}
            className="w-4 h-4 accent-cy-navy"
          />
          <span className="text-[13px] font-semibold text-cy-ink">
            Reduced motion mode
          </span>
          <span className="text-[11px] text-cy-ink-3">
            (shimmer 비활성 — 정적 회색)
          </span>
        </label>
      </section>
...
function ShowcaseSection({
  ...
  return (
    <section className="max-w-[1400px] mx-auto px-6 md:px-12 py-8 border-t border-cy-line">
      <div className="flex items-baseline gap-3 mb-1.5">
        <h2 className="text-[20px] md:text-[22px] font-[800] tracking-[-0.02em] text-cy-ink">
          {name}
        </h2>
        <span
          className="text-[10px] text-cy-ink-3 tracking-[0.2em] uppercase"
          style={{ fontFamily: "var(--font-roboto-mono), monospace" }}
        >
          Component
        </span>
      </div>
      <p className="text-[13px] text-cy-ink-3 mb-5 max-w-[720px]">
        {description}
      </p>

      <pre
        className="bg-cy-ink text-cy-line text-[12px] rounded-xl p-4 mb-5 overflow-x-auto"
        style={{ fontFamily: "var(--font-roboto-mono), monospace" }}
      >
        <code>{code}</code>
      </pre>

      <div className="bg-white border border-cy-line rounded-xl p-6">
        {children}
      </div>
    </section>
  );
...
function ShowcaseNav(): JSX.Element {
  return (
    ...
    >
      ...
      <span
        className="hidden md:inline text-[11px] text-cy-ink-3 tracking-[0.2em] uppercase"
        style={{ fontFamily: "var(--font-roboto-mono), monospace" }}
      >
...
function ShowcaseFooter(): JSX.Element {
  return (
    <footer className="border-t border-cy-line mt-10 pt-8 pb-10 bg-cy-bg-alt">
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 flex flex-col md:flex-row justify-between gap-3">
        <p className="text-[11px] text-cy-ink-3">
