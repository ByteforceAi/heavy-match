...
export default async function AdminDashboard() {
  ...
  const totalRevenue = (commissions as unknown as Array<{ company_fee: number }>)?.reduce((sum, c) => sum + c.company_fee, 0) ?? 0;
  const recent = recentDispatches as unknown as Array<{
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard icon="👥" value={totalUsers ?? 0} label="총 사용자" />
        <StatCard icon="🚧" value={activeDispatches ?? 0} label="진행중 배차" color="text-accent" />
        <StatCard icon="✅" value={completedDispatches ?? 0} label="완료 건수" color="text-success" />
