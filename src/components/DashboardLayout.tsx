  ...
  return (
    <div className="flex h-screen bg-[#f8f9ff]">
      ...
      <aside className="hidden md:flex md:w-64 flex-col bg-white border-r border-[#c1c6d6]/30">
        <div className="p-5 border-b border-[#c1c6d6]/20">
          <div className="flex items-center gap-2">
            <MIcon name="construction" fill className="text-[#0059b9]" />
            <h1 className="text-xl font-black text-[#111c29] tracking-tight">Heavy Match</h1>
          </div>
        ...
        <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <MIcon name={item.icon} fill={isActive} className={isActive ? "text-[#0059b9]" : "text-[#727785]"} />
                {item.label}
              </Link>
            );
          })}
        </nav>
        <div className="p-4 border-t border-[#c1c6d6]/20">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 bg-[#e5eeff] rounded-full flex items-center justify-center text-xs font-bold text-[#0059b9]">
                {userName[0]}
              </div>
              <span className="text-sm font-semibold text-[#111c29] truncate max-w-[120px]">{userName}</span>
            </div>
            <button onClick={handleLogout} className="text-[#727785] hover:text-[#ba1a1a] transition-colors">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-white transform transition-transform duration-300 md:hidden ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="p-5 border-b border-[#c1c6d6]/20 flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2">
              <MIcon name="construction" fill className="text-[#0059b9]" />
              <h1 className="text-xl font-black tracking-tight">Heavy Match</h1>
            </div>
            <p className={`text-xs font-bold mt-1 ${ROLE_COLORS[userRole]}`}>{getRoleLabel(userRole)}</p>
          </div>
          <button onClick={() => setSidebarOpen(false)} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-[#eef4ff]">
            <MIcon name="close" className="text-[#727785]" />
          </button>
        </div>
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-semibold transition-all active:scale-[0.98] ${
                  isActive ? "bg-[#e5eeff] text-[#0059b9]" : "text-[#414754] hover:bg-[#eef4ff]"
                }`}
              >
                <MIcon name={item.icon} fill={isActive} className={isActive ? "text-[#0059b9]" : "text-[#727785]"} />
                {item.label}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-[#c1c6d6]/20 safe-bottom">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* 모바일 헤더 */}
        <header className="md:hidden bg-white/80 backdrop-blur-xl border-b border-[#c1c6d6]/20 px-4 h-14 flex items-center justify-between sticky top-0 z-30">
          <button onClick={() => setSidebarOpen(true)} className="w-10 h-10 flex items-center justify-center rounded-xl hover:bg-[#eef4ff] active:bg-[#e5eeff] transition">
            <MIcon name="menu" className="text-[#111c29]" />
          </button>
          <div className="flex items-center gap-1.5">
            <MIcon name="construction" fill className="text-[#0059b9] text-lg" />
            <span className="font-black text-[#111c29] tracking-tight">Heavy Match</span>
          </div>
          <div className="w-10" />
        </header>

        {/* 하단 탭: 모바일 */}
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-[#c1c6d6]/20 z-30 safe-bottom">
              ...
              return (
                <Link
                  ...
                  className={`flex flex-col items-center justify-center py-2 px-2 rounded-xl min-w-[60px] min-h-[52px] transition-all active:scale-95 ${
                    isActive ? "text-[#0059b9]" : "text-[#727785]"
                  }`}
                >
                  ...
                  {isActive && <span className="w-4 h-0.5 bg-[#0059b9] rounded-full mt-0.5" />}
                </Link>
              );
            })}
          </nav>
